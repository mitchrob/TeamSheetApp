...
@admin_required
def player_names():
    """Return a JSON list of unique player names found in the CSV (sorted).

    This endpoint is protected with `admin_required` because the `/add` form
    is admin-only and the name list is used only for assisting admins.
    """
    counts = compute_appearances()
    names = sorted(counts.keys()) if counts else []
